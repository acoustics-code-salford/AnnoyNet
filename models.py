...
class AnnoyCNN_1Conv(nn.Module):
    def __init__(self, dropout=0.0):
        ...
        self.pool1 = nn.MaxPool2d((16, 32))  # pool time dimension more
        self.bn1 = nn.BatchNorm2d(96)

        # dense layers
        self.fc1 = nn.Linear(6_336, 1_000)
...
class AnnoyCNN_2Conv(nn.Module):
    def __init__(self, dropout=0.0):
        super().__init__()
...
class AnnoyCNN_3Conv(nn.Module):
    def __init__(self, dropout=0.0):
        super().__init__()
        ...
        self.pool3 = nn.MaxPool2d((2, 4))  # pool time dimension more
        self.bn3 = nn.BatchNorm2d(32)

        # dense layers
        self.fc1 = nn.Linear(1_600, 100)
        self.fc2 = nn.Linear(100, 1)

    def forward(self, x):
        x = self.dropout(self.bn1(self.pool1(self.relu(self.conv1(x)))))
        x = self.dropout(self.bn2(self.pool2(self.relu(self.conv2(x)))))
        x = self.dropout(self.bn3(self.pool3(self.relu(self.conv3(x)))))
